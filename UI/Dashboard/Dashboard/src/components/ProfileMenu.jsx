...
export default function ProfileMenu() {
  ...
  useEffect(() => {
    const loadUserData = () => {
      console.log("≡ƒöì ProfileMenu: Loading user data...");
      ...
      if (storedUser) {
        try {
          ...
          console.log('Γ£à User data loaded from storage:', parsedUser);
        } catch (e) {
          console.error('Γ¥î Error parsing user data:', e);
        ...
        console.log('Γ£à User data from session:', { userId, userEmail, userName });
      } else {
        console.warn('ΓÜá∩╕Å No user data found in storage - using fallback');
      }
    };
      ...
      try {
        ...
        if (accessToken) {
          try {
            ...
            console.log('Γ£à Logout API called successfully');
          } catch (error) {
            console.error('Γ¥î Logout API error (continuing anyway):', error);
        ...
        console.log('Γ£à User logged out - storage cleared');
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className="h-9 w-9 rounded-full bg-gradient-to-tr from-indigo-500 to-blue-500 ring-2 ring-slate-700/60 hover:ring-indigo-500/40 transition-shadow flex items-center justify-center text-white text-sm font-medium shadow-lg"
      >
        ...
        <span className={user.profile_picture ? "hidden" : "flex items-center justify-center"}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                  ...
                  <span className={`text-sm ${user.is_verified ? 'text-green-400' : 'text-yellow-400'}`}>
                    {user.is_verified ? 'Γ£ô Verified' : 'Unverified'}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
