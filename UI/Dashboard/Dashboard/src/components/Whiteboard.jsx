...
const Whiteboard = ({
      ...
      try {
        ...
        if (lessonDataStr) {
          ...
          if (lessonId) {
            ...
            const v2Result = await fetchVisualizationV2(lessonId);
            
            if (v2Result) {
              // Check if we got Konva visualization
              if (v2Result.hasVisualization && v2Result.scenes) {
                console.log('  SUCCESS! Using Konva visualization with', v2Result.scenes.length, 'scenes');
                setStatus(` Interactive visualization ready! ${v2Result.scenes.length} scenes prepared.`);
                setTeachingMode(true);
                
                // Load quiz/notes if available
                ...
                console.log('  Konva visualization loaded successfully!');
                return true;
              }
              // Fallback to old teaching steps format
              else if (v2Result.steps && v2Result.steps.length > 0) {
                console.log('  Using converted teaching steps:', v2Result.steps.length);
                setTeachingSteps(v2Result.steps);
                setStatus(` Interactive lesson ready! ${v2Result.steps.length} steps prepared.`);
                setTeachingMode(true);
                ...
                console.log('  Teaching steps loaded successfully!');
                return true;
              }
            } else {
              console.log('  V2 fetch failed, falling back to old format');
            }
          }
          ...
          if (!lessonData.teaching_steps && (lessonData.lesson_content || lessonData.content)) {
            console.log('�  No teaching_steps found, generating from lesson_content...');
  ...
  const startTeachingStep = useCallback(
    (step, stepIndex) => {
      ...
      if (canvasRef.current && typeof canvasRef.current.clearCanvas === 'function') {
        ...
        console.warn(" Canvas clearCanvas method not available");
      }

      // Validate and prepare speech text
      let speechText = step.speech_text;
      if (!speechText || speechText.trim() === "") {
        console.warn(" Empty speech text for step", stepIndex + 1);
      ...
      if (step.drawing_commands && step.drawing_commands.length > 0) {
        ...
        console.warn(" No drawing commands for step", stepIndex + 1);
        console.log(" Step object structure:", Object.keys(step));
        console.log(" step.drawing_commands value:", step.drawing_commands);
      }

      // Start speech with proper completion callback
      console.log(
        `� Starting speech for step ${stepIndex + 1}:`,
      ...
      speak(
        ...
        () => {
          ...
          if (nextStepIndex < currentTeachingSteps.length && autoPlay) {
            console.log(` Auto-advancing to step ${nextStepIndex + 1}`);
            console.log(
              ` Next step object:`,
            ...
            console.log(" Auto-play disabled, waiting for manual advance");
            console.log(
  ...
  useEffect(() => {
    ...
    if (pdfSentRef.current && teachingSteps.length > 0) {
      console.log(' Skipping: Already processed and have teaching steps');
      return;
    }
    ...
    (async () => {
      ...
      if (lessonId && !pdfSentRef.current) {
        ...
        if (v2Steps && v2Steps.length > 0) {
          ...
          console.log('  V2 API fetch failed, trying other sources...');
        }
      }
