...
const KonvaTeachingBoard = ({ scenes = [], onSceneComplete, autoPlay = true }) => {
  ...
  // Load images (enhanced to support URLs and placeholders)
  useEffect(() => {
    ...
    // Find all image and icon shapes
    const imageShapes = currentScene.shapes.filter(shape => 
      (shape.type === 'image' || shape.type === 'icon') && shape.src
    );
    
    if (imageShapes.length === 0) return;

    // Load images with CORS support for external URLs
    imageShapes.forEach(shape => {
      if (!loadedImages[shape.src]) {
        ...
        img.onload = () => {
          console.log(`✅ Loaded image: ${shape.src}`);
  ...
  const renderIcon = (shape, commonProps) => {
    ...
    const pathData = iconPaths[iconName] || iconPaths['lightbulb']; // Default fallback
  ...
  const renderShape = (shape, index) => {
    ...
    switch (shape.type) {
      ...
      case 'icon':
        ...
        return renderIcon(shape, commonProps);
