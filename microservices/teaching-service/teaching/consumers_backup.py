...
from .models import (
    TeachingSessionModel, LessonService, UserService
)
...
class TeachingConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            # Add support for test message types
...
class SessionControlConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_user_message(self, data):
        """Handle user text messages for testing"""
        try:
            message = data.get('message', '')
            ...
            # For temporary sessions, just echo the message with AI response
            if self.session_id.startswith('temp_'):
                ai_response = f"Thank you for your message: '{message}'. This is a test response from the AI teacher!"
    ...
    async def handle_konva_update(self, data):
        """Handle Konva.js whiteboard updates for testing"""
        try:
            konva_data = data.get('konva_data', {})
            
            # Echo back the konva update
            await self.send(text_data=json.dumps({
                'type': 'konva_update',
                'konva_state': konva_data,
                'action': 'update',
                'session_id': self.session_id,
                'timestamp': datetime.now(timezone.utc).isoformat()
            }))
    ...
    async def handle_session_control(self, data):
        """Handle session control commands for testing"""
        try:
            action = data.get('action', data.get('command', ''))
            
            if action in ['start_teaching', 'start', 'pause', 'stop', 'resume']:
                await self.send(text_data=json.dumps({
                    'type': 'session_control_response',
                    'action': action,
                    'status': 'success',
                    'message': f'Session {action} command executed',
                    'session_id': self.session_id,
                    'timestamp': datetime.now(timezone.utc).isoformat()
                }))
            else:
                await self.send_error(f'Unknown session command: {action}')
    ...
    async def handle_get_session_state(self, data):
        ...
        try:
            # For temporary sessions, return a mock session state
            if self.session_id.startswith('temp_'):
                session_state = {
                    'session_id': self.session_id,
                    'status': 'active',
                    ...
                    'lesson_content': {
                        'title': 'Test Session',
                        'content': 'This is a test teaching session.'
                    },
